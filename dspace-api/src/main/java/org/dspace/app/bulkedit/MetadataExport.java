...
public class MetadataExport
{
    ...
    /** GB: whether to include bitstream URLs */
    private boolean includeBitstreams;
    ...
    {
	includeBitstreams = ConfigurationManager.getBooleanProperty("metadata.export.includeBitstreams");
        ...
        try
        {
            ...
            {
		// GB: allow for bitstreams
		if(includeBitstreams){
			// get bitstream URLS
			Item bsItem = toExport.next();
			List<String> bitstreamURLs = getBitstreamURLs(bsItem);	
			// add csv row for each bitstream in this item
			for( String bitstreamURL : bitstreamURLs){
				csv.addItem(bsItem, bitstreamURL);	
			}
		}
		else{
	                csv.addItem(toExport.next(), null);
		}
            }
    ...
    /**
    * GB: Get bitstream URLs from ORIGINAL bundle for item
    *
    * @param itemId The item ID
    * @return bitstreamURLs Array of bitstream URLs
    */
    private List<String> getBitstreamURLs(Item itemId){
	List<String> bitstreamURLs =  new ArrayList<String>();
        // Generate URL
        String dspaceUrl = ConfigurationManager.getProperty("dspace.url");
	try{
          String handle = itemId.getHandle();
          Bundle[] bundles = itemId.getBundles("ORIGINAL");
          int sequenceId = 1;
          String filename = "";
          String url = "";
       		...
       		Bitstream[] bitstreams = bundle.getBitstreams();
                for( Bitstream bitstream: bitstreams ){
               		sequenceId = bitstream.getSequenceID();
                        ...
                        url = dspaceUrl + "/bitstream/" + handle + "/" + sequenceId + "/" + filename;
			// add URL to list
			bitstreamURLs.add(url);
                }
          }
	}
	catch(SQLException sqle){
